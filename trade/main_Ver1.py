...
from maithong_packageV2 import pdatetime , mylib , pyqtHelper as pt
# ,clsCandleArray
...
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QVBoxLayout, \
    ...
    QTabWidget, QMenu, QLabel,QFrame,QCheckBox,QLineEdit,QRadioButton,QGridLayout
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        for i, pair in enumerate(currency_pairs):
            ...
            buttonLabel = ['IDLE','CALL','PUT','WAIT']
            for j in range(4):
                # button = QPushButton(f'Button {j + 1}', self)
                button = QPushButton(buttonLabel[j], self)
        ...
        self.tab_widget.addTab(tab1, "ตารางการเทรด")
        ...
        listOfCaption = ['EUR/USD', 'EUR/GBP', 'EUR/JPY', 'EUR/AUD', 'EUR/CAD']
        listOfVar = ['EURUSD', 'EURGBP', 'EURJPY', 'EURAUD', 'EURCAD']
        ...
        self.caption = QLabel("เลือก สกุลเงิน")
        self.frame_layout.addWidget(self.caption, 0, 0, 1, 5)  # Place the caption at the top-left
        pt.addCheckBoxToFrame(self.frame_layout,listOfVar,listOfCaption)        
        # Set the layout for the QFrame
        self.frame.setLayout(self.frame_layout)        
        self.caption = QLabel("Frame Caption")
        self.frame_layout.addWidget(self.caption)
        pt.addChildToFrame(tab2_layout,self.frame) 
        
        
        listOfCaption = ['EMA3Slope','TurnPoint','EMA3CUTEMA5','CANDLEBODY' ,'MACD', 'ADX', 'STOCH', 'RSI']
        listOfVar = ['EMA3Slope','TurnPoint','EMA3CUTEMA5','CANDLEBODY' ,'MACD', 'ADX', 'STOCH', 'RSI']
        # Create a QFrame
        self.frame = QFrame()
        self.frame.setFrameShape(QFrame.StyledPanel)
        self.frame_layout = QGridLayout()
        self.caption = QLabel("เลือก เทคนิค")
        self.frame_layout.addWidget(self.caption, 0, 0, 1, 5)  # Place the caption at the top-left
        pt.addCheckBoxToFrame(self.frame_layout,listOfVar,listOfCaption)        
        # Set the layout for the QFrame
        self.frame.setLayout(self.frame_layout)        
        ...
        # frameVarName = 'sss'  ; frameCaption = 'List1111'
        # pt.createFrameWithCheckBox(frameVarName,frameCaption,listOfVar,listOfCaption) 
        
        # tab2_layout.addWidget(self.table2)
        
        self.tab_widget.addTab(tab2, "ตั้งค่าต่าง ๆ")
