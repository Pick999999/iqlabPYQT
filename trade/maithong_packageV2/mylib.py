...
def getAnalysisFromCandle(candleAnalysisArray) :
    ...
    return minuteno,color,SlopeDirection,SlopeValue,macd,emaAbove,TurnPoint,isConflict,convergence
    

def DefineBodyCode(UpperHeightPercent,BodyHeightPercent,LowerHeightPercent)  :
    ...
    return UCode + '-' + BodyCode+ '-'+ LowerCode
...
def getEMA35_By_TALIB(candleList,curpair,DataMultiply,tradeid,tradestatus,buyno) :
    ...
    #   candleList[0]['close'],candleList[1]['close'],candleList[2]['close'],candleList[3]['close'],candleList[4]['close'] ,
    #   candleList[5]['close'],candleList[6]['close'],candleList[7]['close'],candleList[8]['close'],candleList[9]['close']       
    #   ])   
    nArray = []
    ...
    if buyno == 0 :
        ...
        # print(candleList[lastIndex]["emaAbove"]  , ' VS ' ,  candleList[lastIndex2]["emaAbove"] )
        ...
        sCode  = DefineBodyCode(candleList[lastIndex]['UpperHeightPercent'],candleList[lastIndex]["BodyHeightPercent"],candleList[lastIndex]['LowerHeightPercent'])   
        print('-=Scode---------------' , sCode)
...
def getNewTradeID(curpair,targetmoney,BalanceOnIQ ) :
    ...
    # tradeid = 1111 ; sectionno =1 
    print('รอบแรก ', tradeid,' :: ',sectionno ,' Balance ตอนเช้า = ',balanceOnMorning )
    # sectionno = 1
    
    # tradeid = tradeResult.text
    # sectionno = 1
...
def getActionMultitrade():
    
    
    url = 'https://lovetoshopmall.com/iqlab/updateFromTradeview.php' 
    myobj = {
     'Mode': 'getTradeAction'    
    }
    x = requests.post(url, json = myobj)
...
def MultipleTrade(I_want_money,tradeList) :
    
    # ************ ทดสอบ Multi Trade **********************

# tradeList= getAction()
# # tradeList = ''
# MultipleTrade(tradeList)

# ************ End  ทดสอบ Multi Trade **********************
     
    tradeList2 = tradeList.split('|')
    curpairAr = [] ; actionAr= [] 
    ACTIVES=[]
    ACTION=[]
    for i in range(len(tradeList2)) :
        print(tradeList2[i])
        tradelist3= tradeList2[i].split('=')
        curpairAr.append(tradelist3[0])
        actionAr.append(tradelist3[1])
    ...
    Money.append(1)
    ACTIVES.append(curpairAr[0])
    ACTION.append(actionAr[0])#put
    expirations_mode.append(1)

    Money.append(1)
    ACTIVES.append(curpairAr[1])
    ACTION.append(actionAr[1])#put
    expirations_mode.append(1)
    
    Money.append(1)
    ACTIVES.append(curpairAr[2])
    ACTION.append(actionAr[2])#put
    expirations_mode.append(1)
    
    Money.append(1)
    ACTIVES.append(curpairAr[3])
    ACTION.append(actionAr[3])#put
    expirations_mode.append(1)
    ...
    # print("check win only one id (id_list)")
    profit1 = I_want_money.check_win_v3(id_list[0])
    print(profit1)
    profit2 = I_want_money.check_win_v3(id_list[1])
    print(profit2)
    
    profit3 = I_want_money.check_win_v3(id_list[2])
    print(profit3)
    
    profit4 = I_want_money.check_win_v3(id_list[3])
    print(profit4)
    # sys.exit(0)
    # profit3 = I_want_money.check_win_v3(id_list[2])
    # print(profit2)
    ...
    balance = 0 
    # print('Start Check Profit')
    # for i in range(len(id_list)-1) :
    #     profit1 = I_want_money.check_win_v3(id_list[i])
    #     print(ACTIVES[i] , ' = ', profit1 )
        
    # print(I_want_money.check_win_v3(id_list[0]))
    # print(I_want_money.check_win_v3(id_list[1]))    